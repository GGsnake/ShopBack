 ...
 */
@RestController
@RequestMapping("/sys/daygoods")
public class SysJhDayGoosGContrller extends AbstractController {
     ...
     */
    @RequestMapping("/list")
    @RequiresPermissions("sys:daygoods:list")
    public Result list(@RequestParam Map<String, Object> params) {
        //查询列表数据
        Query query = new Query(params);
        List<SysDaygoods> sysDaygoods = daygoodsDao.queryList(query);
...
//
    ...
    public Result save(HttpServletRequest request, @RequestParam("file") MultipartFile[] myfile, SysDaygoods sysDaygoods) {
        ...
        Integer flag = daygoodsDao.saveDayGoods(map);
        if (flag == 0) {
            return Result.error("创建失败请重试");
        }
        SysDaygoods tempGoods = daygoodsDao.queryId(sysDaygoods.getContent());
        if (tempGoods==null){
            return Result.error("创建失败请重试");
        }
...
//
	...
	@RequiresPermissions("sys:daygoods:delete")
