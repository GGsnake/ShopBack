$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '图片内容url数组', name: 'contentImages', index: 'content_images', width: 80 }, 			
			{ label: '删除状态', name: 'status', index: 'status', width: 80 }, 			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function () {
			var url = vm.daygoods.id == null ? "sys/daygoods/save" : "sys/daygoods/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.daygoods),
			    success: function(r){
			    	if(r.code === 0){
						alert('操作成功', function(){
							vm.reload();
